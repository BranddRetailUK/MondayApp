...
- The Monday Dashboard print label opens a 4in by 6in print window with job number, customer, job title, and a scanner QR code. Label values still shrink to fit horizontally, but the job number, customer, and job title start from capped maximum font sizes so short text cannot expand into the QR area. After the browser print dialog completes and focus returns, the label print popup closes itself.
...
- Test Dashboard parent rows show a grey three-dot action button immediately to the left of the Print label button. Clicking it opens a small row menu whose first item is `Move To`; hovering that item opens a submenu for Hold, Office, and Pre-Production. Choosing a group writes dashboard-only move state through `/api/test-dashboard/items/:jobId/group`; Pre-Production moves are accepted only after `JOB ✔` approval. The same menu includes `Remove Proof`, which clears the row's Test Dashboard `PROOF` files through `/api/test-dashboard/items/:jobId/proof-files`.
...
- Test Dashboard group placement follows the copied board automation rules from DB state on every render after approval: `STATUS = READY TO PRINT` routes by the DB-derived `TYPE` (`PRINT` to `PRINT`, `EMB` / `EMB / PRINT` to `EMBROIDERY`), `STATUS = COMPLETED` marks the dashboard status complete but does not set the legacy closed/invoiced flag, `HOLD` routes to `HOLD`, `TO SAMPLE` to `TO SAMPLE`, `SAMPLED` to `OFFICE`, and approved `NO STOCK` / `PRE-PRODUCTION` to `PRE-PRODUCTION`. `STOCK ORDERED` updates the status only and preserves the current non-production group until `JOB ✔` approval. User, scanner, and Stock Ordering report status changes update both `test_dashboard_job_state` and the `database_jobs` dashboard fields, so DATABASE and Test Dashboard stay aligned. Status changes do not infer or overwrite job approval; `JOB ✔` is the approval driver and the only normal move out of OFFICE for new jobs. When a Test Dashboard user selects `COMPLETED` in the `STATUS` picker, the frontend shows an are-you-sure modal before sending the update.
...
- Test Dashboard label printing posts `/api/test-dashboard/items/:jobId/label-printed`, builds the 4in by 6in label in an off-screen iframe, and immediately calls the browser print API without opening a label preview popup. The event returns and prints a `/test-scan` QR code for DB-backed rows and applies the label-printed status automation before printing. Normal browser security still shows native print confirmation; launching the dedicated dashboard Chrome profile with `npm run launch:tuesday-print` enables Chromium kiosk printing so the click sends the label to the macOS default printer without a second confirmation. Scanner input while the Test Dashboard tab is active expands bare numeric job ids through the test scan-url route and posts scan results to `/api/test-dashboard/scanner`.
...
- `GET /api/database/jobs?q=&customer=&type=&year=&status=&limit=&offset=&includeTotal=`: returns paginated jobs. Default and UI page size is 100 jobs. `includeTotal=false` skips the count query for follow-up page loads when the UI already has the first-page total. Supported `status` filters include `open`, `complete` / `completed`, and `to-invoice`; `complete` / `completed` uses the imported legacy closed flag, while `to-invoice` means `dashboard_status = COMPLETED` with the legacy closed flag false, invoice required not false, and invoice/pro-forma printed flags not true. Search covers job number, invoice number, customer, job title, contact name/email, client order number, line description, style code, style name, colour, and size. The list response also includes imported order flags and timestamps used by the legacy outstanding-orders grid, plus a normalized `dashboardStatusColors` map copied from the Tuesday Dashboard `STATUS` settings.
...
- `GET /api/database/users`: returns Hub users, including registration timestamps, for DATABASE customer account-manager dropdowns and the Users admin page.
- `DELETE /api/database/users/:id`: deletes one Hub user row from `hub_users`; matching `hub_sessions` rows are removed by the existing `ON DELETE CASCADE` relationship.
...
- Jobs: `tblOrder` joined to `tblCustomer`, `tblContact`, and `tblOrderType`. `tblOrder.ynComplete` imports to `database_jobs.is_complete`; despite the current column name, it is the legacy closed flag, and closed jobs are treated as already invoiced.
- Manual New Order rows store non-MDB form fields directly on `database_jobs`: `delivery_method`, `payment_terms`, `order_taken_by`, `order_owner_user_id`, `order_owner_name`, `delivery_address`, `invoice_address`, `is_manual_entry`, and the initial Test Dashboard `dashboard_status`.
...
- `npm run launch:tuesday-print -- [dashboard-url]`: opens the Tuesday Dashboard in a dedicated macOS Google Chrome app profile with `--kiosk-printing`. It defaults to the production Tuesday Dashboard URL, can be overridden with `TUESDAY_DASHBOARD_URL`, and prints to the macOS default printer using that printer's default settings without showing Chrome's second print confirmation.
