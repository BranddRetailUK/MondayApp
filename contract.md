...
- The Test Dashboard parent rows are keyed by `database_jobs.source_order_id`; subitems are populated from `database_job_line_items`; design text comes from `database_job_positions` plus job screen-number fields; open jobs exclude `database_jobs.is_complete = true`, `invoice_printed = true`, `pf_invoice_printed = true`, and `dashboard_status = INVOICED`, and currently exclude Business Gifts jobs until a dedicated gifts group is added. Job `STATUS`, `PRIORITY`, `TYPE`, and proof approval are persisted on `database_jobs.dashboard_status`, `dashboard_priority`, `dashboard_type`, and `proof_approved`; Test Dashboard reads these DB fields before falling back to seeded Monday JSON state. When no seeded/manual group state exists, unseeded DB jobs fall back to the same Printing/Embroidery/Business Gifts categorization as `/api/database/outstanding-counts`, placing print jobs in `PRINT`, embroidery jobs in `EMBROIDERY`, and other non-gift jobs in `OFFICE`.
- Test Dashboard group placement follows the copied board automation rules from DB state on every render: `STATUS = READY TO PRINT` routes by `TYPE` (`PRINT` to `PRINT`, `EMB` / `EMB / PRINT` to `EMBROIDERY`), `STATUS = COMPLETED` routes to `COMPLETED`, `HOLD` to `HOLD`, `TO SAMPLE` to `TO SAMPLE`, `SAMPLED` to `OFFICE`, and `NO STOCK` / `PRE-PRODUCTION` to `PRE-PRODUCTION`. User and scanner status changes update both `test_dashboard_job_state` and the `database_jobs` dashboard fields, so DATABASE and Test Dashboard stay aligned. `proof_approved` is set true for statuses that imply approval, such as `READY TO PRINT`, `IN PRODUCTION`, and `COMPLETED`, and false for `WAITING APPROVAL` / not-approved statuses.
