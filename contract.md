...
- The Test Dashboard parent rows are keyed by `database_jobs.source_order_id`; subitems are populated from `database_job_line_items`; design text comes from `database_job_positions` plus job screen-number fields; open jobs exclude `database_jobs.is_complete = true` and currently exclude Business Gifts jobs until a dedicated gifts group is added. Job `STATUS`, `PRIORITY`, `TYPE`, and proof approval are persisted on `database_jobs.dashboard_status`, `dashboard_priority`, `dashboard_type`, and `proof_approved`; Test Dashboard reads these DB fields before falling back to seeded Monday JSON state. When no seeded/manual group state exists, unseeded DB jobs fall back to the same Printing/Embroidery/Business Gifts categorization as `/api/database/outstanding-counts`, placing print jobs in `PRINT`, embroidery jobs in `EMBROIDERY`, and other non-gift jobs in `OFFICE`.
