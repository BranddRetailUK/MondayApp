...
- `PUT /api/test-dashboard/items/:jobId/checkbox-column`: updates DB-backed Test Dashboard parent checkbox columns in `test_dashboard_job_state.column_values`. Body: `{ "columnId": "...", "checked": true|false }`. The route accepts only mirrored checkbox columns. Toggling the `JOB ✔` approval checkbox also updates `database_jobs.proof_approved` and `proof_approved_at`.
- `PUT /api/test-dashboard/items/:jobId/design-column`: appends missing DES/PSG values from the Test Dashboard to the related DATABASE Design tab rows. Body: `{ "columnId": "...", "value": "..." }`. Submitted values are split on `/`; each side is inserted as its own `database_job_positions` row when that represented design/PSG reference is not already present. Exact represented design/PSG references are not inserted twice.
...
- The Test Dashboard parent rows are keyed by `database_jobs.source_order_id`; subitems are populated from `database_job_line_items`; DES/PSG text is derived from `database_job_positions.design_ref` plus the job screen-number field, with duplicate design refs collapsed and displayed as `design number(s) / PSG or stitch references` when both are present, or PSG/stitch references first when no design ref exists. Design refs always render before PSG/stitch references on the dashboard even when PSG rows sit above design rows in DATABASE. DATABASE is authoritative for DES/PSG updates/removals: older `test_dashboard_job_state` DES/PSG values are treated only as a one-time migration fallback when no DATABASE design or screen reference exists, then cleared from test-dashboard state after backfill; if DATABASE already has a reference, the stale state value is cleared without being reinserted. Open jobs exclude the legacy closed/invoiced flag `database_jobs.is_complete = true`, `invoice_printed = true`, `pf_invoice_printed = true`, and `dashboard_status = INVOICED` / `COMPLETED`, and currently exclude Business Gifts jobs until a dedicated gifts group is added. Test Dashboard jobs must also have dashboard identity from `database_jobs.dashboard_status` / `dashboard_priority`, scanner status, or a `test_dashboard_job_state` row seeded from Monday/manual board state; stale imported DB-only jobs with no dashboard identity are not rendered. Job `STATUS`, `PRIORITY`, and proof approval are persisted on `database_jobs.dashboard_status`, `dashboard_priority`, and `proof_approved`; Test Dashboard TYPE is derived first from `database_jobs.order_type` / `order_type_abbr`, mapping Printing to `PRINT`, Embroidery to `EMB`, and Print + Emb to `EMB / PRINT`, before falling back to seeded state only when the DB order type cannot be categorized.
- Test Dashboard group placement follows the copied board automation rules from DB state on every render: `STATUS = READY TO PRINT` routes by the DB-derived `TYPE` (`PRINT` to `PRINT`, `EMB` / `EMB / PRINT` to `EMBROIDERY`), `STATUS = COMPLETED` marks the dashboard status complete but does not set the legacy closed/invoiced flag, `HOLD` routes to `HOLD`, `TO SAMPLE` to `TO SAMPLE`, `SAMPLED` to `OFFICE`, and `NO STOCK` / `PRE-PRODUCTION` to `PRE-PRODUCTION`. User and scanner status changes update both `test_dashboard_job_state` and the `database_jobs` dashboard fields, so DATABASE and Test Dashboard stay aligned. `proof_approved` is set true for statuses that imply approval, such as `READY TO PRINT`, `IN PRODUCTION`, and `COMPLETED`, and false for `WAITING APPROVAL` / not-approved statuses. When a Test Dashboard user selects `COMPLETED` in the `STATUS` picker, the frontend shows an are-you-sure modal before sending the update.
- Test Dashboard parent checkbox cells are clickable and persist their checked/unchecked state to the DB-backed board state without touching Monday. Checkbox clicks update the visible tick immediately and keep that optimistic state through board refreshes while the save is in flight; the UI reverts only if the API update fails. The `JOB ✔` checkbox is the approval tick; checking it sets `database_jobs.proof_approved = true`, and clearing it sets `proof_approved = false`.
- The Test Dashboard DES/PSG parent cell is editable. Pressing Enter or leaving the field saves a non-empty changed value through `/api/test-dashboard/items/:jobId/design-column`, which adds missing values as new DATABASE Design rows for that order. A submitted `/` creates separate DATABASE design rows, so `12345 / PSG7543` stores `12345` and `PSG7543` separately while the dashboard still displays them in the normal design-first format. Clearing the cell in the Test Dashboard UI does not delete existing DATABASE design rows.
...
- `POST /api/database/jobs`: creates a real manual `database_jobs` row from the legacy New Order form. Required fields are customer, order type, job title, order date, delivery date, and invoice required. The route accepts Business Gifts, Printing, Print + Emb, and Embroidery order types, deriving `order_type_abbr` as `G`, `P`, `PE`, or `E`. It allocates the next source order id and next job/order number from `MAX(source_order_id) + 1` and `GREATEST(MAX(order_no), 50000) + 1`. If `invoice_required = true`, it allocates `invoice_no` separately from `GREATEST(MAX(invoice_no), 50000) + 1`; if `invoice_required = false`, it stores `invoice_no = NULL` and does not consume an invoice number. The route marks the row `is_manual_entry = true` and seeds `dashboard_status = PRE-PRODUCTION` so new DB-created jobs have Test Dashboard identity.
...
- Dashboard status DB fields on `database_jobs`: `dashboard_status`, `dashboard_priority`, `dashboard_type`, `proof_approved`, `proof_approved_at`, and `dashboard_status_updated_at`. Full DATABASE replace imports preserve and restore these fields by `source_order_id`.
