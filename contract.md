...
- Parent rows render the retained `Print` action button, the job name with a subitem-count badge when present, then live Monday columns. Status columns render colored Monday-style badges using Monday status settings when available; Monday checkbox columns render blank when unchecked and standalone colored ticks when checked; file columns render Monday attachments through the asset proxy where possible, so the `PROOF` column shows the file attached to Monday; text, date, people, and timeline columns render with type-specific formatting.
