...
- Test Dashboard group placement follows the copied board automation rules from DB state on every render: `STATUS = READY TO PRINT` routes by the DB-derived `TYPE` (`PRINT` to `PRINT`, `EMB` / `EMB / PRINT` to `EMBROIDERY`), `STATUS = COMPLETED` marks the dashboard status complete but does not set the legacy closed/invoiced flag, `HOLD` routes to `HOLD`, `TO SAMPLE` to `TO SAMPLE`, `SAMPLED` to `OFFICE`, and `NO STOCK` / `PRE-PRODUCTION` to `PRE-PRODUCTION`. User and scanner status changes update both `test_dashboard_job_state` and the `database_jobs` dashboard fields, so DATABASE and Test Dashboard stay aligned. `proof_approved` is set true for statuses that imply approval, such as `READY TO PRINT`, `IN PRODUCTION`, and `COMPLETED`, and false for `WAITING APPROVAL` / not-approved statuses.
