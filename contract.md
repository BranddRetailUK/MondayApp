...
- The Test Dashboard parent rows are keyed by `database_jobs.source_order_id`; subitems are populated from `database_job_line_items`; design text comes from `database_job_positions` plus job screen-number fields; open jobs exclude `database_jobs.is_complete = true`, `invoice_printed = true`, `pf_invoice_printed = true`, and `dashboard_status = INVOICED` / `COMPLETED`, and currently exclude Business Gifts jobs until a dedicated gifts group is added. Test Dashboard jobs must also have dashboard identity from `database_jobs.dashboard_status` / `dashboard_priority`, scanner status, or a `test_dashboard_job_state` row seeded from Monday/manual board state; stale imported DB-only jobs with no dashboard identity are not rendered. Job `STATUS`, `PRIORITY`, and proof approval are persisted on `database_jobs.dashboard_status`, `dashboard_priority`, and `proof_approved`; Test Dashboard TYPE is derived first from `database_jobs.order_type` / `order_type_abbr`, mapping Printing to `PRINT`, Embroidery to `EMB`, and Print + Emb to `EMB / PRINT`, before falling back to seeded state only when the DB order type cannot be categorized.
- Test Dashboard group placement follows the copied board automation rules from DB state on every render: `STATUS = READY TO PRINT` routes by the DB-derived `TYPE` (`PRINT` to `PRINT`, `EMB` / `EMB / PRINT` to `EMBROIDERY`), `STATUS = COMPLETED` closes the database job by setting `is_complete = true`, `HOLD` routes to `HOLD`, `TO SAMPLE` to `TO SAMPLE`, `SAMPLED` to `OFFICE`, and `NO STOCK` / `PRE-PRODUCTION` to `PRE-PRODUCTION`. User and scanner status changes update both `test_dashboard_job_state` and the `database_jobs` dashboard fields, so DATABASE and Test Dashboard stay aligned. `proof_approved` is set true for statuses that imply approval, such as `READY TO PRINT`, `IN PRODUCTION`, and `COMPLETED`, and false for `WAITING APPROVAL` / not-approved statuses.
...
- The To Invoice home button opens a DATABASE page listing completed jobs that still need invoicing, sourced from `/api/database/jobs?status=to-invoice`. The list shows order number, customer, type, job title, completed date, and invoice number, and each row opens the normal order detail view.
