...
- The Outstanding Orders tab loads open jobs through `/api/database/jobs?status=open`, follows pagination until all open jobs are loaded, and groups rows into Print, Embroidery, Gifts, and Other using `order_type`/`order_type_abbr`. The All Orders view uses the same 100-row page size but renders the first page immediately, then continues loading remaining pages silently in the background with `includeTotal=false` follow-up requests so full-history imports do not block the initial screen paint. In All Orders mode, order selector dropdowns are capped to the first 500 loaded rows plus the currently selected order to avoid rendering tens of thousands of options.
