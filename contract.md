...
- Test Dashboard subitem grids render a display-only computed `TOTAL` row at the bottom when a subitem `QTY`/`QUANTITY` column exists; the row sums all non-`TOTAL` subitem quantities and replaces any source-provided `TOTAL` row for display only.
