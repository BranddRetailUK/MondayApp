...
- Test Dashboard parent rows show a grey three-dot action button immediately to the left of the Print label button. Clicking it opens a small row menu whose first item is `Move To`; hovering that item opens a submenu for Hold, Office, and Pre-Production. Choosing a group writes dashboard-only move state through `/api/test-dashboard/items/:jobId/group`.
- The OFFICE group shows a `+` control to the left of the group title/closed-card title. Clicking it creates a dashboard-only private job row in OFFICE with an editable free-text title and no generated job number. Private rows can use the normal Test Dashboard proof upload, DES/PSG, NOTES, checkbox, status, date, priority, and automation controls, but they are stored only in `test_dashboard_private_jobs` and never appear in DATABASE or invoice flows.
