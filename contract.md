...
- Critical invoice invariant: every source-backed `database_jobs.invoice_no` must match `tblOrder.lngInvoiceNo` from the current MDB snapshot exactly, including `NULL` invoice values. Manual Hub-created jobs are not present in the MDB; their invoice numbers must be allocated after the source MDB max invoice and must not duplicate any MDB invoice number.
...
Verify source-backed DB invoice numbers against the current MDB:
...
- `npm run verify:mdb-invoices -- [PS_XP_tab.mdb] [--max-mismatches=25]`: fail if source-backed `database_jobs.invoice_no` values differ from `tblOrder.lngInvoiceNo` in the supplied MDB, or if any DB invoice number is duplicated.
