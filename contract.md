...
- `PUT /api/test-dashboard/items/:jobId/design-column`: appends a missing DES/PSG value from the Test Dashboard to the related DATABASE Design tab rows by inserting a `database_job_positions` row with `design_ref` set to the submitted value. Body: `{ "columnId": "...", "value": "..." }`. Exact represented design/PSG references are not inserted twice.
...
- The Test Dashboard DES/PSG parent cell is editable. Pressing Enter or leaving the field saves a non-empty changed value through `/api/test-dashboard/items/:jobId/design-column`, which adds the value as a new DATABASE Design row for that order. Clearing the cell in the Test Dashboard UI does not delete existing DATABASE design rows.
