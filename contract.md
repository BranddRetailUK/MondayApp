...
- `POST /api/database/jobs`: creates a real manual `database_jobs` row from the legacy New Order form. Required fields are customer, order type, job title, order date, and delivery date. The route allocates the next source order id plus one shared document number in a transaction, where the document number is `GREATEST(MAX(order_no), MAX(invoice_no), 50000) + 1`. It writes that shared value to both `order_no` and `invoice_no`, including when `invoice_required = false`, and marks the row `is_manual_entry = true`.
