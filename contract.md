...
- `POST /api/database/jobs`: creates a real manual `database_jobs` row from the legacy New Order form. Required fields are customer, order type, job title, order date, delivery date, and invoice required. The route accepts Business Gifts, Printing, Print + Emb, and Embroidery order types, deriving `order_type_abbr` as `G`, `P`, `PE`, or `E`. It allocates the next source order id and next job/order number from `MAX(source_order_id) + 1` and `GREATEST(MAX(order_no), 50000) + 1`. Manual order creation does not allocate or reserve an invoice number; `invoice_no` stays `NULL` until the Invoice action generates the invoice. The route marks the row `is_manual_entry = true` and seeds `dashboard_status = AWAITING APPROVAL` so new DB-created jobs have Test Dashboard identity and land in OFFICE until approved.
