...
- `PUT /api/board/items/:itemId/status-column`: updates one parent item Monday status-type column from the dashboard. Requires Hub API auth and Monday auth. Body: `{ "columnId": "...", "label": "..." }`. The route only accepts dashboard parent columns titled `STATUS` or `PRIORITY`, plus the configured `STATUS_COLUMN_ID` for compatibility, verifies the label exists in that column's Monday `settings_str`, writes via `change_column_value`, and clears the `/api/board` cache. `/api/board/items/:itemId/job-status` is retained as an alias for the same handler.
