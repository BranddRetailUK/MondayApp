...
- Test Dashboard parent checkbox cells are clickable and persist their checked/unchecked state to the DB-backed board state without touching Monday. The `JOB ✔` checkbox is the approval tick; checking it sets `database_jobs.proof_approved = true`, and clearing it sets `proof_approved = false`.
