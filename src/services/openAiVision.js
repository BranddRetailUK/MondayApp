...
function buildPrompt({ side = 'front', jobTitle = '', jobNo = '', garmentColour = '' }) {
  return [
    ...
    `Focus ONLY on the common artwork/logo that appears in both images (e.g., the brand mark + brand text).`,
    `Explicitly IGNORE: measurements, guides, annotations, background props, or any text/elements that exist in only one image.`,
    `Primary checks: missing letters/words, incorrect words, spelling/grammar errors, wrong layout/ordering of the shared artwork text, obvious rotation/flip/placement errors of that artwork.`,
    `Secondary (only if obvious): major positional errors of the shared artwork. Ignore colour/shade differences unless they change the legibility of the shared artwork.`,
    `If the shared artwork matches, return a high confidence. Only lower confidence when you see real differences in the shared artwork.`,
